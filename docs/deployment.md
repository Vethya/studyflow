...
| `STUDYFLOW_GOOGLE_OIDC_REDIRECT_URI` | no | `render.yaml` | `https://<api-host>/api/v1/auth/google/callback`; must be HTTPS |
...
4. Note the service URL (for example `https://studyflow-api.onrender.com`). If Google
   Sign-In will be used, register `<service-url>/api/v1/auth/google/callback` in the Google
   Cloud console and fill the three OIDC variables.
